...
class TiingoAggregator:
    ...
    def get_market_movers_comprehensive(
        self, limit: int = 20, include_extended_hours: bool = True
    ) -> Dict[str, List[ExtendedMarketMover]]:
        """
        ...
        """
        results = {"gainers": [], "losers": [], "most_active": []}
        ...
        try:
            ...
            if most_active_result.success:
                ...
                logger.info(
                    f"Retrieved {len(most_active_result.data)} most active from Tiingo"
                )
            else:
    ...
    def _fetch_tiingo_gainers(self, limit: int) -> SourceResult:
        ...
        try:
            movers = self.coordinator.get_market_movers(
                "tiingo", "gainers", limit=limit
            )
            ...
            return SourceResult(
                "tiingo_gainers", True, extended_movers, fetch_time=datetime.now()
            )
        except Exception as e:
    ...
    def _fetch_tiingo_losers(self, limit: int) -> SourceResult:
        ...
        try:
            ...
            return SourceResult(
                "tiingo_losers", True, extended_movers, fetch_time=datetime.now()
            )
        except Exception as e:
    ...
    def _fetch_tiingo_most_active(self, limit: int) -> SourceResult:
        ...
        try:
            movers = self.coordinator.get_market_movers(
                "tiingo", "most_active", limit=limit
            )
            ...
            return SourceResult(
                "tiingo_most_active", True, extended_movers, fetch_time=datetime.now()
            )
        except Exception as e:
    ...
    def get_extended_hours_movers(
        ...
        try:
            ...
            logger.info(
                "Extended hours movers functionality will be implemented with enhanced Tiingo API integration"
            )
    ...
    def get_aggregation_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            "rate_limits": {"calls_per_minute": 1000, "tier": "commercial"},
