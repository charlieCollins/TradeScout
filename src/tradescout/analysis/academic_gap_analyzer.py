...
from ..data_models.domain_models_core import ExtendedHoursData, MarketQuote, MarketStatus
...
class AcademicGapTypeAnalyzer(CandidateGapTypeAnalyzer):
    ...
    def classify_gap_type(
        ...
        gap_amount = quote.price_data.price - quote.previous_close if quote.previous_close else Decimal("0")
        ...
        classification = GapClassification(
            ...
            supporting_factors=self._generate_classification_reasons(final_type, gap_size, quote),
            warning_flags=self._identify_warning_flags(quote, gap_size)
        )
        
        logger.info(f"Classified {quote.asset.symbol} as {final_type.value} gap "
    ...
    def analyze_gap_strength(
        ...
        """
        logger.debug(f"Analyzing gap strength for {gap_classification.gap_type.value} gap")
        ...
        strength_metrics = GapStrengthMetrics(
            ...
            support_resistance_break=market_context.get("support_resistance_break", False),
            news_catalyst_present=market_context.get("news_catalyst", False),
            catalyst_quality_score=Decimal(str(self._calculate_strength_score(volume_strength, technical_strength, catalyst_strength) / 100)),
            market_alignment=market_context.get("market_alignment", False),
            sector_momentum=market_context.get("sector_momentum", False),
            overall_strength=overall_strength,
            strength_score=Decimal(str(self._calculate_strength_score(volume_strength, technical_strength, catalyst_strength) / 100))
        )
        
        logger.info(f"Gap strength analysis: {overall_strength.value} "
    ...
    def assess_tradability(
        ...
        """
        logger.debug(f"Assessing tradability for {gap_classification.gap_type.value} gap")
        ...
        entry_timing = self._recommend_entry_timing(gap_classification, strength_metrics)
        
        # Generate key success factors
        success_factors = self._identify_success_factors(gap_classification, strength_metrics)
        
        # Generate risk warnings
        risk_warnings = self._generate_risk_warnings(gap_classification, strength_metrics)
        
        # Determine recommended trade side
        recommended_side = TradeSide.LONG if gap_classification.gap_percent > 0 else TradeSide.SHORT
        
        # Map strength to risk level
        risk_level = GapRiskLevel.LOW if strength_metrics.overall_strength == GapStrength.VERY_STRONG else \
                     GapRiskLevel.MEDIUM if strength_metrics.overall_strength == GapStrength.STRONG else \
                     GapRiskLevel.HIGH
        ...
        logger.info(f"Tradability assessment: {'TRADEABLE' if is_tradeable else 'NOT TRADEABLE'} "
    ...
    def batch_analyze_candidates(
        ...
        for quote in candidates:
            try:
                ...
                extended_data = ExtendedHoursData(
                    ...
                    regular_session_close=quote.previous_close or Decimal("100.0")  # Fallback if missing
                )
        ...
        logger.info(f"Batch analysis complete: {tradeable_count}/{len(assessments)} tradeable")
    ...
    def _determine_risk_level(
        ...
        if (gap_classification.gap_type in [GapType.BREAKAWAY, GapType.CONTINUATION] and
            overall_strength in [GapStrength.STRONG, GapStrength.VERY_STRONG]):
    ...
    def _determine_tradability(
        self, gap_classification: GapClassification, strength_metrics: GapStrengthMetrics
    ) -> bool:
    ...
    def _identify_warning_flags(self, quote: MarketQuote, gap_size: Decimal) -> List[str]:
        ...
        if hasattr(quote.asset, 'market_cap') and quote.asset.market_cap and quote.asset.market_cap < 1_000_000_000:
