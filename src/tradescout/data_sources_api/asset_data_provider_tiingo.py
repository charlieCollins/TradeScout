...
class AssetDataProviderTiingo(AssetDataProvider):
    ...
    def __init__(self, api_key: str, cache: Optional[APICache] = None, request_delay: float = 0.1):
        """
    ...
    def _fetch_current_quote(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            price_data = latest_price[-1] if isinstance(latest_price, list) else latest_price
    ...
    def _convert_to_market_quote(self, quote_data: Dict[str, Any], asset: Asset) -> MarketQuote:
        ...
        try:
            ...
            current_price = Decimal(str(price_data.get("close", price_data.get("adjClose", 0))))
            ...
            change_percent = (price_change / open_price * 100) if open_price > 0 else Decimal("0")

            # Create PriceData with required fields first
            price_data_obj = PriceData(
                asset=asset,
                timestamp=datetime.fromisoformat(price_data["date"].replace('Z', '+00:00')),
                price=current_price,
                volume=volume,
    ...
    def get_extended_hours_data(
        ...
        try:
            ...
            current_price = (
                iex_data.get('tngoLast') or 
                iex_data.get('mid') or 
                iex_data.get('bidPrice')
            )
    ...
    def get_historical_prices(
        ...
        try:
            ...
            return [self._convert_to_price_data(price_record) for price_record in historical_data]

        except Exception as e:
            logger.error(f"Error getting historical prices for {asset.symbol}: {e}")
            return []

    def _fetch_historical_data(self, symbol: str, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Error fetching historical data from Tiingo for {symbol}: {e}")
            return []

    def _convert_to_price_data(self, price_record: Dict[str, Any]) -> PriceData:
        """Convert Tiingo historical price record to PriceData"""
        try:
            close_price = Decimal(str(price_record.get("close", price_record.get("adjClose", 0))))
    ...
    def _screen_market_movers(self, mover_type: str, limit: int) -> List[MarketMover]:
        ...
        try:
            ...
            logger.info(f"Screening {len(screening_symbols)} symbols for real-time {mover_type}")
    ...
    def get_company_info(self, asset: Asset) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            metadata = cached_api_call(
                ...
                api_function=lambda: self.client.get_ticker_metadata(asset.symbol, fmt='json'),
