...
class AssetDataProviderFinnhub(AssetDataProvider):
    ...
    def get_current_quote(self, asset: Asset) -> Optional[MarketQuote]:
        ...
        try:
            ...
            logger.debug(
                f"  current_price: {quote_data['current_price']} (type: {type(quote_data['current_price'])})"
            )
            logger.debug(
                f"  previous_close: {quote_data['previous_close']} (type: {type(quote_data['previous_close'])})"
            )
    ...
    def get_extended_hours_data(
        ...
        logger.info(
            f"Extended hours data not available in Finnhub free tier for {asset.symbol}"
        )
        return None
    ...
    def get_historical_quotes(
        ...
        try:
            ...
            logger.error(
                f"Error getting Finnhub historical data for {asset.symbol}: {e}"
            )
            return []
    ...
    def scan_volume_leaders(
        ...
        return sorted(
            volume_leaders, key=lambda q: abs(q.price_change_percent or 0), reverse=True
        )
        ...
        try:
            ...
            def fetch_fundamentals() -> Dict[str, Any]:
                ...
                fundamentals = {
                    ...
                    "market_cap": (
                        data.get("marketCapitalization", 0) * 1000000
                        if data.get("marketCapitalization")
                        else 0
                    ),  # Convert from millions
                    "shares_outstanding": (
                        data.get("shareOutstanding", 0) * 1000000
...
def create_asset_data_provider_finnhub(
