...
from ..data_models.interfaces import AssetDataProvider, DataCollectionCoordinator
...
class MultiProviderCoordinator(DataCollectionCoordinator):
    ...
    def add_provider(
        self, name: str, provider: AssetDataProvider, priority: int = 1
    ) -> None:
        """
            ...
            try:
                ...
                if quote:
                    result["quotes"][provider_name] = {
                        ...
                        "change": (
                            float(quote.price_change) if quote.price_change else 0
                        ),
                        "change_percent": (
                            float(quote.price_change_percent)
                    ...
                    logger.info(
                        f"Successfully got quote from {provider_name}: ${quote.price_data.price}"
                    )
                else:
    ...
    def collect_overnight_data(self) -> Dict[str, Any]:
        ...
        if successful_quotes:
            overnight_data["summary"]["average_change_percent"] = total_change / len(
                successful_quotes
            )

        # Find top movers
        price_movers = sorted(
            successful_quotes, key=lambda x: abs(x["change_percent"]), reverse=True
        )[:5]
        ...
        volume_leaders = sorted(
            successful_quotes, key=lambda x: x["volume"], reverse=True
        )[:5]
