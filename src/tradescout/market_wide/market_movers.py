...
class MarketMoversProvider(MarketWideDataProvider):
    ...
    def get_most_active(
        ...
        if self.has_alpha_vantage:
            try:
                ...
                if most_active:
                    logger.info(
                        f"Retrieved {len(most_active)} most active from Alpha Vantage"
                    )
                    return most_active
            except Exception as e:
    ...
    def get_market_movers_report(
        ...
        if self.has_alpha_vantage:
            try:
                report = self.alpha_vantage.get_market_movers_report(
                    limit, force_refresh
                )
                if report:
                    logger.info(
                        "Retrieved complete market movers report from Alpha Vantage"
                    )
                    return report
            except Exception as e:
...
def create_market_movers_provider(
...
if __name__ == "__main__":
    ...
    for gainer in gainers:
        print(
            f"  {gainer.asset.symbol}: {gainer.price_change_percent:.2f}% (Rank: {gainer.rank})"
        )
    ...
    for loser in losers:
        print(
            f"  {loser.asset.symbol}: {loser.price_change_percent:.2f}% (Rank: {loser.rank})"
        )
    ...
    print(
        f"Gainers: {len(report.gainers)}, Losers: {len(report.losers)}, Most Active: {len(report.most_active)}"
    )
