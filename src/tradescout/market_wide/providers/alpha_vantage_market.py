...
class AlphaVantageMarketProvider:
    ...
    def get_market_movers_report(
        ...
        try:
            ...
            most_active = self._process_movers_list(
                raw_data.get("most_actively_traded", []), limit
            )
    ...
    def _process_movers_list(
        self, raw_movers: List[Dict], limit: int
    ) -> List[MarketMover]:
        """
        ...
        for i, mover_data in enumerate(raw_movers[:limit]):
            try:
                ...
                asset = Asset(
                    ...
                    name=mover_data.get(
                        "ticker", ""
                    ),  # Alpha Vantage doesn't provide company name
                ...
                change_percent = self._safe_decimal(
                    mover_data.get("change_percentage", "0%").rstrip("%")
                )
                ...
                logger.warning(
                    f"Error processing mover {mover_data.get('ticker', 'unknown')}: {e}"
                )
                continue
...
def create_alpha_vantage_market_provider(
...
if __name__ == "__main__":
    ...
    if report:
        ...
        for gainer in report.gainers[:3]:
            print(
                f"  {gainer.asset.symbol}: {gainer.price_change_percent:.2f}% (${gainer.current_price})"
            )

        print("\\n🔴 Top Losers:")
        for loser in report.losers[:3]:
            print(
                f"  {loser.asset.symbol}: {loser.price_change_percent:.2f}% (${loser.current_price})"
            )

        print("\\n📊 Most Active:")
        for active in report.most_active[:3]:
            print(
                f"  {active.asset.symbol}: {active.volume:,} shares (${active.current_price})"
            )
    else:
