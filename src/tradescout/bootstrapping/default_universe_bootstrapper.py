...
from ..config.universe_config import should_include_in_default_universe, get_default_universe_config
...
class DefaultUniverseBootstrapper:
    ...
    def bootstrap_default_universe(self, dry_run: bool = False) -> Dict[str, int]:
        ...
        logger.info(f"Found {len(qualified_assets)} assets that meet default universe criteria")
        
        # Get current universe membership to avoid duplicates
        current_universe_assets = set(self.universe_manager.get_universe_assets("default_universe"))
        new_assets = [asset for asset in qualified_assets if asset['ticker'] not in current_universe_assets]
        ...
        if dry_run:
            ...
            self._log_sample_assets(new_assets[:10], "Sample assets that would be added:")
            return stats
        ...
        if not universe:
            ...
            self.universe_manager.create_universe(
                config["name"], 
                config["description"]
            )
        ...
        for asset_data in new_assets:
            try:
                ...
                logger.error(f"Error adding {asset_data.get('ticker', 'unknown')} to universe: {e}")
                stats['errors'] += 1
    ...
    def get_universe_stats(self) -> Dict[str, int]:
        ...
        try:
            ...
            universe_assets = len(self.universe_manager.get_universe_assets("default_universe"))
            universe_stats = self.universe_manager.get_universe_statistics("default_universe")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Bootstrap default universe from existing assets')
    ...
    try:
        ...
        if args.stats_only:
            ...
            print(f"  Total assets in database: {stats.get('total_assets_in_db', 'Unknown')}")
            print(f"  Assets in default universe: {stats.get('default_universe_assets', 'Unknown')}")
            
            universe_stats = stats.get('default_universe_stats', {})
            if universe_stats:
                print(f"  Active assets in universe: {universe_stats.get('active_assets', 'Unknown')}")
                print(f"  First asset added: {universe_stats.get('first_asset_added', 'Unknown')}")
                print(f"  Last asset added: {universe_stats.get('last_asset_added', 'Unknown')}")
        else:
