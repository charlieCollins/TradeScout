...
class DataSourcesManager:
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            for data_type_id, data_type_data in yaml_data.get("data_types", {}).items():
                fallback_strategy = FallbackStrategy(
                    data_type_data.get("fallback_strategy", "first_success")
                )
    ...
    def get_providers_for_data_type(
        ...
        for provider_id in data_config.providers:
            ...
            if provider_id in self.disabled_providers:
                ...
                if datetime.now() - disable_time < timedelta(
                    minutes=10
                ):  # 10 minute timeout
    ...
    def get_fallback_strategy(
        self, data_type: Union[str, DataSourceType]
    ) -> FallbackStrategy:
    ...
    def record_provider_failure(self, provider_id: str) -> None:
        ...
        failure_window = timedelta(
            minutes=self.config.error_handling.get("failure_window_minutes", 10)
        )
        self.provider_failures[provider_id] = [
            failure_time
            for failure_time in self.provider_failures[provider_id]
        ...
        if len(self.provider_failures[provider_id]) >= max_failures:
            ...
            logger.warning(
                f"Disabled provider {provider_id} due to {max_failures} failures"
            )
