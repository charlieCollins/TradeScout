...
class ProviderConfigManager:
    ...
    def create_coordinator(
        ...
        for provider_name, config, is_available in sorted_providers:
            ...
            try:
                provider_instance = self._create_provider_instance(
                ...
                if provider_instance:
                    ...
                    logger.info(
                        f"Added {provider_name} provider with priority {priority}"
                    )
    ...
    def _create_provider_instance(self, provider_name: str, config: dict):
        ...
        if provider_name == "polygon":
            ...
            try:
                from ..data_sources.asset_data_provider_alpha_vantage import (
    ...
    def validate_configuration(self) -> List[str]:
        ...
        for provider_name, config, is_available in available_providers:
            if provider_name != "yfinance" and not is_available:
                issues.append(
                    f"{provider_name} API key not configured (set {provider_name.upper()}_API_KEY)"
                )

        # Check for conflicting priorities
        priorities = [
            config.get("priority", 10)
            for _, config, available in available_providers
            if available
        ]
        if len(priorities) != len(set(priorities)):
            issues.append(
                "Multiple providers have the same priority - may cause unpredictable ordering"
            )
...
def create_polygon_first_coordinator() -> MultiProviderCoordinator:
    ...
    manager = get_provider_config_manager()
    return manager.create_coordinator(
        preferred_providers=["polygon", "yfinance", "alpha_vantage"]
    )
