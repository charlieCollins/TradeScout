...
class APICache:
    ...
    def get(
        ...
        endpoint: str,
        params: Dict[str, Any],
        policy: CachePolicy = CachePolicy.INTRADAY,
    ...
    def set(
        ...
        try:
            ...
            self.stats["saves"] += 1
            logger.debug(
                f"Cache SAVE: {provider}:{endpoint} (TTL: {self.config.ttl_policies[policy]}m)"
            )
    ...
    def cached_api_call(
        ...
        Args:
            ...
            policy: Cache policy
            force_refresh: Bypass cache and make fresh API call
    ...
    def invalidate(
        self, provider: str = None, endpoint: str = None, symbol: str = None
    ) -> int:
        """
            ...
            for cache_file in provider_dir.glob("*.json"):
                ...
                try:
                    ...
                    if endpoint and cache_entry.get("endpoint") == endpoint:
                        ...
                        symbol
                        and symbol.upper() in str(cache_entry.get("params", {})).upper()
                    ):
                        should_remove = True
...
def cached_api_call(
    ...
    return get_api_cache().cached_api_call(
        provider, endpoint, params, api_function, policy, force_refresh
    )
