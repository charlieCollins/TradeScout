"""Ticker snapshot caching using asset_prices table and data_update_metadata."""

import logging
from typing import Optional, Dict, Any, Callable
from datetime import datetime
from models.snapshot import TickerSnapshot
from models.data_update_metadata import DataUpdateMetadataType
from config.ttl_config import TICKER_SNAPSHOT_TTL_MINUTES
from .base_cache_manager import BaseCacheManager

logger = logging.getLogger(__name__)


class TickerSnapshotCache(BaseCacheManager):
    """Cache ticker snapshot operations using asset_prices table with operation-level TTL validation."""
    ...
    def get_entity_from_database(self, key: str) -> Optional[TickerSnapshot]:
        ...
        try:
            with self.db_manager.get_connection() as conn:
                ...
                # Construct TickerSnapshot from database data
                ticker_data = {
                    'ticker': symbol,
                    'updated': row[1],  # updated_at
                    'prevDay': {
                        'o': float(row[2]) if row[2] else None,  # prevday_open
                        'h': float(row[3]) if row[3] else None,  # prevday_high
                        'l': float(row[4]) if row[4] else None,  # prevday_low
                        'c': float(row[5]) if row[5] else None,  # prevday_close
                        'v': int(row[6]) if row[6] else None,    # prevday_volume
                        'vw': float(row[7]) if row[7] else None  # prevday_vwap
                    },
                    'day': {
                        'o': float(row[8]) if row[8] else None,  # day_open
                        'h': float(row[9]) if row[9] else None,  # day_high
                        'l': float(row[10]) if row[10] else None, # day_low
                        'c': float(row[11]) if row[11] else None, # day_close
                        'v': int(row[12]) if row[12] else None,   # day_volume
                        'vw': float(row[13]) if row[13] else None # day_vwap
                    },
                    'min': {
                        't': int(row[14]) if row[14] else None,  # min_timestamp
                        'o': float(row[15]) if row[15] else None, # min_open
                        'h': float(row[16]) if row[16] else None, # min_high
                        'l': float(row[17]) if row[17] else None, # min_low
                        'c': float(row[18]) if row[18] else None, # min_close
                        'v': int(row[19]) if row[19] else None,   # min_volume
                        'vw': float(row[20]) if row[20] else None # min_vwap
                    }
                }

                # Create TickerSnapshot from the constructed data
                return TickerSnapshot.from_polygon_data(ticker_data)
