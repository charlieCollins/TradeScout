...
from data_collection.domain_models_analysis import (
    ...
    PerformanceMetrics, MarketEvent
)
...
class QuoteRepository(ABC):
    ...
    @abstractmethod
    def get_quotes_by_timeframe(self, symbol: str, start_time: datetime, 
                              end_time: datetime) -> List[MarketQuote]:
        """
...
class ExtendedHoursRepository(ABC):
    ...
    @abstractmethod
    def get_pre_market_gaps(self, min_gap_percent: Decimal, 
                          date: Optional[datetime] = None) -> List[ExtendedHoursData]:
        """
...
class NewsRepository(ABC):
    ...
    @abstractmethod
    def get_news_by_timeframe(self, start_time: datetime, 
                            end_time: datetime) -> List[NewsItem]:
        """Get news within time range"""
        pass
    
    @abstractmethod
    def search_news_by_keywords(self, keywords: List[str], 
    ...
    @abstractmethod
    def get_news_sentiment_summary(self, symbol: str, 
...
class SentimentRepository(ABC):
    ...
    @abstractmethod
    def get_sentiment_timeline(self, symbol: str, start_time: datetime,
                             end_time: datetime) -> List[SocialSentiment]:
...
class TechnicalRepository(ABC):
    ...
    @abstractmethod
    def get_latest_indicators(self, symbol: str, timeframe: str) -> Optional[TechnicalIndicators]:
        """Get latest technical indicators"""
        pass
    
    @abstractmethod
    def get_indicator_history(self, symbol: str, timeframe: str, 
                            days_back: int) -> List[TechnicalIndicators]:
...
class SuggestionRepository(ABC):
    ...
    def update_suggestion_performance(self, suggestion_id: str, 
                                    max_profit: Decimal, max_loss: Decimal,
                                    current_price: Decimal) -> bool:
        """Update suggestion with current performance"""
        pass
    
    @abstractmethod
    def get_suggestion_performance_history(self, days_back: int = 30) -> List[TradeSuggestion]:
        """Get suggestion performance history"""
        pass
    
    @abstractmethod
    def get_suggestions_by_confidence(self, min_confidence: Decimal) -> List[TradeSuggestion]:
...
class TradeRepository(ABC):
    ...
    def get_trades_by_date_range(self, start_date: datetime, 
                               end_date: datetime) -> List[ActualTrade]:
        """Get trades within date range"""
        pass
    
    @abstractmethod
    def update_trade_exit(self, trade_id: str, exit_price: Decimal,
                        exit_time: datetime, exit_reason: str) -> bool:
...
class EventRepository(ABC):
    ...
    @abstractmethod
    def get_upcoming_events(self, symbol: str, days_ahead: int = 7) -> List[MarketEvent]:
    ...
    @abstractmethod
    def get_earnings_calendar(self, start_date: datetime, 
                            end_date: datetime) -> List[MarketEvent]:
...
class DatabaseManager(ABC):
    ...
    @abstractmethod
    def execute_raw_query(self, query: str, params: Optional[List] = None) -> List[Dict]:
