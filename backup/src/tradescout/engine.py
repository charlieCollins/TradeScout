...
from .data_models import Asset, AssetType, PriceData
from .data_sources.data_provider_coordinator import DataProviderCoordinator
...
class TradeScoutEngine:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        self.db_manager = SQLiteDatabaseManager(db_path or "storage/tradescout.db")

        # Initialize data provider coordinator
        self.coordinator = DataProviderCoordinator(db_manager=self.db_manager)

    def _get_current_trading_session(self, market: str = "nasdaq") -> TradingSession:
        ...
        if premarket_start <= current_time < regular_start:
            ...
            # Overnight hours (8 PM - 4 AM): extended hours
            return TradingSession.AFTERHOURS

    # Market Data Methods

    def display_quotes(self, symbols: List[str], force_refresh: bool = False) -> Table:
        """
        ...
        """
        # Force refresh market snapshot if requested
        if force_refresh:
            self.coordinator.get_market_snapshot(force_refresh=True)
        ...
        for symbol in symbols:
            ...
            if quote:
                ...
                # Format data
                price = f"${quote.current_price:.2f}" if quote.current_price else "N/A"
                ...
                volume = f"{quote.volume:,}" if quote.volume else "0"
                timestamp = quote.timestamp.strftime("%-I:%M %p")
        ...
        if (
            ...
            if hasattr(provider, "get_market_data_status"):
                ...
                if status["status"] == "cached":
                    source = status.get("source", "database")
                    time_left = status.get("time_left_minutes", 0)
                    if time_left > 0:
                        return f"[cyan]💾 Market Data: {status['symbols']:,} symbols from {source} cache ({status['age_minutes']:.1f}m old, {time_left:.1f}m remaining)[/cyan]"
                    else:
                        return f"[cyan]💾 Market Data: {status['symbols']:,} symbols from {source} cache ({status['age_minutes']:.1f}m old)[/cyan]"
                elif status["status"] == "empty":
                    return "[cyan]🔄 Market Data: Fetching fresh snapshot from Polygon API...[/cyan]"
                elif status["status"] == "stale":
                    source = status.get("source", "database")
                    return f"[yellow]⚠️  Market Data: {status['symbols']:,} symbols from {source} (stale, refreshing from API...)[/yellow]"
                else:
                    return f"[cyan]📊 Market Data: {status['symbols']:,} symbols[/cyan]"
        return ""
    ...
    def display_gainers(self, limit: int = 10, force_refresh: bool = False) -> List:
        ...
        """
        try:
            current_session = self._get_current_trading_session("nasdaq")
            now = datetime.now()
            ...
            table.add_column("Market Cap", justify="right", style="dim")
            table.add_column("Time", style="dim", no_wrap=True)
            ...
            for gainer in gainers:
                ...
                timestamp_str = gainer.timestamp.strftime("%-I:%M %p") if gainer.timestamp else "N/A"

                # Format market cap if available
                if gainer.market_cap:
                    if gainer.market_cap >= 1_000_000_000_000:  # Trillion
                        market_cap_str = f"${gainer.market_cap / 1_000_000_000_000:.1f}T"
                    elif gainer.market_cap >= 1_000_000_000:  # Billion
                        market_cap_str = f"${gainer.market_cap / 1_000_000_000:.1f}B"
                    elif gainer.market_cap >= 1_000_000:  # Million
                        market_cap_str = f"${gainer.market_cap / 1_000_000:.1f}M"
                    else:
                        market_cap_str = f"${gainer.market_cap:,.0f}"
                else:
                    market_cap_str = "N/A"
    ...
    def display_losers(self, limit: int = 10, force_refresh: bool = False) -> List:
        ...
        try:
            ...
            for loser in losers:
                ...
                timestamp_str = loser.timestamp.strftime("%-I:%M %p") if loser.timestamp else "N/A"

                # Format market cap if available
                if loser.market_cap:
                    if loser.market_cap >= 1_000_000_000_000:  # Trillion
                        market_cap_str = f"${loser.market_cap / 1_000_000_000_000:.1f}T"
                    elif loser.market_cap >= 1_000_000_000:  # Billion
                        market_cap_str = f"${loser.market_cap / 1_000_000_000:.1f}B"
                    elif loser.market_cap >= 1_000_000:  # Million
                        market_cap_str = f"${loser.market_cap / 1_000_000:.1f}M"
                    else:
                        market_cap_str = f"${loser.market_cap:,.0f}"
                else:
                    market_cap_str = "N/A"
    ...
    def get_trade_suggestions(
        ...
        Returns:
            List of GapAssessment objects
        """
        try:
            result = self.coordinator.get_daily_gap_suggestions(
                min_gap_percent=min_gap, movers_limit=limit
            )
            return result.get("suggestions", []) if isinstance(result, dict) else result
        except Exception as e:
            return []
    ...
    def analyze_single_symbol_gap(
        ...
        try:
            ...
            market_data = provider.get_market_snapshot(force_refresh)
            if not market_data or symbol not in market_data:
                display_objects.append(
                    f"[red]❌ No market data available for {symbol}[/red]"
                )
                return display_objects
            ...
            if not prev_close:
                display_objects.append(
                    f"[red]❌ No previous close data for {symbol}[/red]"
                )
                return display_objects
            ...
            if "min" in ticker_data and ticker_data["min"]:
                current_price = ticker_data["min"].get("c", 0) or ticker_data[
                    "min"
                ].get("o", 0)
                volume = ticker_data["min"].get("v", 0)

            # Fallback to day data
            if not current_price and "day" in ticker_data and ticker_data["day"]:
                current_price = ticker_data["day"].get("c", 0) or ticker_data[
                    "day"
                ].get("o", 0)
            ...
            if not current_price:
                display_objects.append(
                    f"[red]❌ No current price available for {symbol}[/red]"
                )
                return display_objects
            ...
            # Create detail table
            table = Table(
                title=f"Gap Analysis for {symbol}",
                show_header=True,
            ...
            table.add_row(
                "Gap Direction",
                f"[{'green' if gap_direction == 'UP' else 'red'}]{gap_direction}[/]",
            )
            ...
            if abs(gap_percent) >= 2.0:
                ...
                asset = Asset(
                    ...
                    name=f"{symbol} Corp",  # Placeholder name
                    market=market,
                    currency="USD",
                )
                ...
                # Simple gap analysis (single responsibility)
                recommendation = self._analyze_gap_opportunity(
                    abs(gap_percent), volume, current_price, gap_direction
                )

                if recommendation["is_tradeable"]:
                    display_objects.append(
                        Panel(
                            f"[green]✅ POTENTIAL GAP TRADE[/green]\n"
                    ...
                    display_objects.append(
                        Panel(
                            f"[yellow]⚠️ NOT RECOMMENDED[/yellow]\n"
    ...
    def _analyze_gap_opportunity(
        self, gap_percent: float, volume: int, current_price: float, gap_direction: str
    ) -> Dict[str, str]:
        """
        Simple single-responsibility gap analysis

        Args:
            gap_percent: Absolute gap percentage
        ...
        """
        # Simple academic research-based criteria
        is_tradeable = False
        strategy = "None"
        confidence = "Low"
        risk_level = "High"
        ...
        if gap_percent >= 2.0:  # Minimum 2% gap
            if volume > 0:  # Has volume
                if current_price >= 1.0:  # Price above $1
                    if gap_percent < 10.0:  # Not too extreme (likely manipulation)
                        is_tradeable = True
                        strategy = f"Gap {'Fill' if gap_percent > 5.0 else 'Continuation'} - {gap_direction}"

                        # Confidence based on gap size
                        if 2.0 <= gap_percent <= 4.0:
                            confidence = "Medium"
                            risk_level = "Medium"
                        elif 4.0 < gap_percent <= 7.0:
                            confidence = "High"
                            risk_level = "Medium"
                        else:  # > 7%
                            confidence = "Medium"
                        ...
                        reason = f"Gap too extreme ({gap_percent:.1f}% > 10%)"
                else:
                    reason = f"Price too low (${current_price:.2f} < $1.00)"
            else:
                reason = "No volume data"
        else:
            reason = f"Gap too small ({gap_percent:.1f}% < 2.0%)"
    ...
    def display_trade_suggestions(
        ...
        try:
            ...
            # Use limit for movers_limit if provided, otherwise use default
            movers_limit = limit if limit is not None else 100
            suggestion_result = self.coordinator.get_daily_gap_suggestions(
                min_gap_percent=min_gap, movers_limit=movers_limit
            )
            suggestions = (
            ...
            for i, gap_assessment in enumerate(suggestions, 1):
                # Get data from GapAssessment object
                gap_candidate = gap_assessment.gap_candidate
                gap_percent = float(gap_candidate.gap_percent)
                gap_color = "green" if gap_percent > 0 else "red"

                # Determine direction based on gap direction and strategy
                direction = (
                    "📈 Long" if gap_candidate.gap_direction == "up" else "📉 Short"
                )

                # Convert confidence level to percentage-like display
                confidence_map = {"very_high": 95, "high": 85, "medium": 70, "low": 50}
                confidence = confidence_map.get(gap_assessment.confidence.value, 50)

                table.add_row(
                    str(i),
                    gap_candidate.asset.symbol,
    ...
    def display_market_movers(
        ...
        try:
            ...
            if formatted_gainers:
                ...
                gainers_table.add_column("Market Cap", justify="right", style="dim")
                gainers_table.add_column("Time", style="dim", no_wrap=True)
            ...
            if formatted_losers:
                ...
                losers_table.add_column("Market Cap", justify="right", style="dim")
                losers_table.add_column("Time", style="dim", no_wrap=True)
        ...
        if verbose:
            try:
                ...
                if polygon_status.get("available"):
                    ...
                    display_messages.append(
                        "[dim]Polygon provider: ❌ Unavailable[/dim]"
                    )
    ...
    def _get_session_info(
        ...
        # Format current time
        current_time_str = timestamp.strftime("%Y-%m-%d %-I:%M:%S %p EST")
