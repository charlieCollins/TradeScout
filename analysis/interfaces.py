...
from data_collection.domain_models_core import (
    Asset, MarketQuote, ExtendedHoursData, NewsItem, SocialSentiment
)
from data_collection.domain_models_analysis import (
    TechnicalIndicators, TradeSuggestion, ActualTrade,
    PerformanceMetrics, MarketEvent, ConfidenceLevel, TradeSide
)
...
class MomentumDetector(ABC):
    ...
    @abstractmethod
    def analyze_gap_momentum(self, quote: MarketQuote, extended_data: ExtendedHoursData) -> Dict[str, any]:
        """
    ...
    @abstractmethod
    def analyze_volume_momentum(self, quote: MarketQuote, historical_volume: List[int]) -> Dict[str, any]:
        """
    ...
    @abstractmethod
    def analyze_news_momentum(self, symbol: str, news: List[NewsItem], 
                            sentiment: Optional[SocialSentiment]) -> Dict[str, any]:
        """
    ...
    @abstractmethod
    def calculate_momentum_score(self, symbol: str, analysis_data: Dict[str, any]) -> Decimal:
        """
...
class TechnicalAnalyzer(ABC):
    ...
    @abstractmethod
    def calculate_support_resistance(self, quotes: List[MarketQuote]) -> Tuple[Decimal, Decimal]:
        """
    ...
    @abstractmethod
    def is_favorable_setup(self, indicators: TechnicalIndicators, 
                          momentum_score: Decimal) -> bool:
        """
...
class RiskCalculator(ABC):
    ...
    @abstractmethod
    def calculate_stop_loss(self, entry_price: Decimal, side: TradeSide,
                          volatility: Decimal, support_resistance: Tuple[Decimal, Decimal]) -> Decimal:
        """
    ...
    @abstractmethod
    def calculate_take_profit_levels(self, entry_price: Decimal, stop_loss: Decimal,
                                   side: TradeSide, target_ratio: Decimal = Decimal(1.5)) -> Tuple[Decimal, Decimal]:
        """
    ...
    def validate_risk_reward(self, entry: Decimal, stop: Decimal, 
                           take_profit: Decimal, min_ratio: Decimal = Decimal(1.0)) -> bool:
        """
...
class SuggestionEngine(ABC):
    ...
    @abstractmethod
    def generate_suggestion(self, symbol: str, analysis_data: Dict[str, any]) -> Optional[TradeSuggestion]:
        """
    ...
    @abstractmethod
    def rank_suggestions(self, suggestions: List[TradeSuggestion]) -> List[TradeSuggestion]:
        """
    ...
    @abstractmethod
    def filter_suggestions(self, suggestions: List[TradeSuggestion],
...
class PerformanceTracker(ABC):
    ...
    @abstractmethod
    def update_suggestion_outcome(self, suggestion_id: str, 
                                max_profit: Decimal, max_loss: Decimal) -> None:
        """
    ...
    @abstractmethod
    def calculate_period_performance(self, start_date: datetime, 
...
class AlertManager(ABC):
    ...
    @abstractmethod
    def send_morning_report(self, suggestions: List[TradeSuggestion],
...
class MarketScanner(ABC):
    ...
    @abstractmethod
    def scan_pre_market_gaps(self, min_gap_percent: Decimal = Decimal(1.0)) -> List[MarketQuote]:
        """
    ...
    @abstractmethod
    def scan_volume_spikes(self, min_volume_ratio: Decimal = Decimal(2.0)) -> List[MarketQuote]:
        """
    ...
    @abstractmethod
    def scan_news_catalysts(self, max_age_hours: int = 24) -> List[Tuple[str, List[NewsItem]]]:
        """
