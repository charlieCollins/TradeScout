...
from src.tradescout.data_models.models_base import Market
...
class TestDataProviderPolygon:
    ...
    def test_get_current_quote(self, provider, mock_polygon_response):
        ...
        with patch.object(provider, 'get_market_snapshot') as mock_snapshot:
            ...
            assert quote.price_data.price == Decimal("150.25")
            assert quote.price_data.volume == 12345  # From min data
    ...
    def test_get_most_active(self, provider, mock_polygon_response):
        """Test getting most active stocks"""
        with patch.object(provider, 'get_market_snapshot') as mock_snapshot:
            mock_snapshot.return_value = {
                "AAPL": mock_polygon_response["results"][0],  # Volume: 45,678,900
                "TSLA": mock_polygon_response["results"][1],  # Volume: 23,456,789
            }
            
            most_active = provider.get_most_active(limit=10)
            
            assert len(most_active) == 2
            # Should be sorted by volume (AAPL first, then TSLA)
            assert most_active[0].asset.symbol == "AAPL"
            assert most_active[0].volume == 45678900
            assert most_active[0].rank == 1
            assert most_active[1].asset.symbol == "TSLA"
            assert most_active[1].volume == 23456789
            assert most_active[1].rank == 2
    ...
    def test_get_historical_data(self, mock_get, provider):
        ...
        historical_data = provider.get_historical_data("AAPL", start_date, end_date)
        
        assert len(historical_data) == 1
        price_data = historical_data[0]
        assert isinstance(price_data, PriceData)
        assert price_data.asset.symbol == "AAPL"
        assert price_data.price == Decimal("150.25")
        assert price_data.volume == 45678900
