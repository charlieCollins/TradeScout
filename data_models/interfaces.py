...
from .domain_models_core import (
    ...
    ExtendedHoursData, NewsItem, SocialSentiment, MarketStatus
)
from .domain_models_analysis import (
    TradeSuggestion, ActualTrade, PerformanceMetrics, MarketEvent,
    TechnicalIndicators
)


class MarketDataProvider(ABC):
    """Abstract interface for market data providers (Polygon, yfinance, etc.)"""
    ...
    @abstractmethod
    def get_extended_hours_data(self, asset: Asset, session: MarketStatus) -> Optional[ExtendedHoursData]:
        """
    ...
    @abstractmethod
    def get_historical_quotes(self, asset: Asset, start_date: datetime, 
                            end_date: datetime, interval: str = "1d") -> List[PriceData]:
        """
    ...
    @abstractmethod
    def scan_volume_leaders(self, assets: List[Asset], min_volume_ratio: Decimal = Decimal(2.0)) -> List[MarketQuote]:
        """
...
class NewsProvider(ABC):
    ...
    @abstractmethod
    def get_news_by_timeframe(self, symbols: List[str], start_time: datetime, 
    ...
    @abstractmethod
    def search_news_by_keywords(self, keywords: List[str], limit: int = 50) -> List[NewsItem]:
        """
...
class SentimentProvider(ABC):
    ...
    @abstractmethod
    def get_sentiment_data(self, symbol: str, lookback_hours: int = 24) -> Optional[SocialSentiment]:
        """
...
class TechnicalAnalysisProvider(ABC):
    ...
    @abstractmethod
    def calculate_indicators(self, symbol: str, timeframe: str = "1d") -> Optional[TechnicalIndicators]:
        """
...
class EventProvider(ABC):
    ...
    @abstractmethod
    def get_upcoming_events(self, symbols: List[str], days_ahead: int = 7) -> List[MarketEvent]:
        """
    ...
    @abstractmethod
    def get_earnings_calendar(self, start_date: datetime, end_date: datetime) -> List[MarketEvent]:
        """
