...
class YFinanceScanner:
    ...
    def get_after_hours_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            market_close = hist.index[-1].replace(hour=16, minute=0, second=0, microsecond=0)
            after_hours_end = market_close + timedelta(hours=4)
            
            after_hours_data = hist[
                (hist.index > market_close) & 
                (hist.index <= after_hours_end)
            ]
            ...
            return {
                ...
                'after_hours_change': float(after_hours_data['Close'].iloc[-1] - after_hours_data['Open'].iloc[0]),
                'timestamp': datetime.now()
            }
    ...
    def get_pre_market_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            pre_market_data = hist[
                (hist.index >= today) & 
                (hist.index < market_open)
            ]
            ...
            gap_percent = ((current_price - previous_close) / previous_close * 100) if previous_close > 0 else 0
    ...
    def scan_volume_leaders(self, symbols: List[str], min_volume_ratio: float = 2.0) -> List[Dict[str, Any]]:
        """
        ...
        return sorted(volume_leaders, key=lambda x: x.get('volume_ratio', 0), reverse=True)
