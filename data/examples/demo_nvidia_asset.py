...
from tradescout.data_models.factories import MarketFactory, MarketSegmentFactory
from tradescout.data_models.domain_models_analysis import TechnicalIndicators
...
def print_nvidia_asset_visualization():
    ...
    print("=" * 60)
    print(
        f"📊 NVIDIA Corporation Analysis - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
    )
    print()
    ...
    for segment in sorted(nvidia.segments, key=lambda s: s.segment_type):
        hierarchy = (
            " → ".join(segment.full_hierarchy)
            if hasattr(segment, "full_hierarchy")
            else segment.name
        )
    ...
    print("-" * 30)
    print(
        f"Gap Status: {'📈 Gap Up' if quote.is_gap_up else '📉 Gap Down' if quote.is_gap_down else '➡️ Normal'}"
    )
    print(f"Volume Surge: {'🔥 YES' if quote.has_volume_surge else '❌ No'}")
    print(f"Session: {quote.price_data.session_type.value}")
    print(
        f"Near ATH: {'✅ YES' if quote.price_data.price >= Decimal('174.00') else '❌ No'}"
    )
    print()
    ...
    print("-" * 30)
    print(
        f"RSI (14): {technicals.rsi} {'🔴 Overbought' if technicals.is_overbought else '🟢 Normal' if not technicals.is_oversold else '🔵 Oversold'}"
    )
    print(
        f"MACD: {technicals.macd} (Signal: {technicals.macd_signal}) {'🔥 Bullish' if technicals.is_macd_bullish else '❄️ Bearish'}"
    )
    print(f"SMA 20: ${technicals.sma_20}")
    print(f"SMA 50: ${technicals.sma_50}")
    print(
        f"Price vs SMA20: {((quote.price_data.price / technicals.sma_20 - 1) * 100):+.1f}%"
    )
    print(
        f"Price vs SMA50: {((quote.price_data.price / technicals.sma_50 - 1) * 100):+.1f}%"
    )
    print()
    ...
    print(
        f"Primary Segment: {nvidia.primary_segment.name if nvidia.primary_segment else 'N/A'}"
    )
